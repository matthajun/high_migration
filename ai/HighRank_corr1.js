...
module.exports.searchAndtransm = async function() {
    schedule.scheduleJob('10 * * * * *', async function() {
        ...
        //     table_name = \'motie_ai_corr_result_v2\'`;  //인서트된 버전의 최대값
        // let version = await clickhouse.query(version_query).toPromise();  //인서트된 버전의 최대값
        // if(version[0].a_time.length) { // 샐렉트 결과가 있을 때
        ...
        // const query = `select * from dti.motie_ai_corr_result_v2 where version >= '${a_time}' and version < '${b_time}'`;
        ...
        try {
            ...
            if (rslt instanceof Error) {
                ...
                HistoryInsert.history_insert(rslt.length, 'motie_ai_corr_result_v2')
            }
        } catch (error) {
